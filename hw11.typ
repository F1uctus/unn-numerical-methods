...
уравнение $y' = f(x, y)$:
$
  1 / (8h) (y_k - 3 y_(k - 2) + 2 y_(k - 3))
  = 1 / 2 (f_(k - 1) + f_(k - 2)).
$

*Решение.*
Для проверки перейдём к анализу локальной невязки схемы
...
$
  1 / 2 (f_(k - 1) + f_(k - 2))
  = 1 / 2 ( (y'_k - h y''_k + (h^2) / 2 y'''_k)
    + (y'_k -2h y''_k +2h^2 y'''_k) )
  = #hl($y'_k - (3 h) / 2 y''_k + (5 h^2) / 4 y'''_k + O(h^3).$)
$
...
$
  (A f_(n + 1) + B f_n + C f_(n - 1)) / D.
$

Подставим в схему разложение для $f_(n+1)$:
$
...
// Надо дальше заменить везде A <-> B, C <-> D.
...
Теперь выберем $D$, чтобы коэффициенты были удобными,
например, $D=12$. Тогда правая часть:
$
